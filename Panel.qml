...
Panel {
  ...
  property bool adaptiveActive: false
  ...
  readonly property string adaptiveStatusText: {
    ...
    // Only a limit raised all the way to 100% is the real pre-unplug phase;
    // any other mismatch is a setting change the daemon hasn't applied yet.
    if (live === 100 && holdCeiling < 100) return "charging to 100% before you unplug"
    if (live !== holdCeiling) return "applying " + holdCeiling + "%..."
    if (adaptiveBatteryState === "Charging") return "charging to " + holdCeiling + "%"
    return "holding at " + holdCeiling + "%"
  }
  ...
  function refresh() {
    ...
    if (!adaptiveProc.running) adaptiveProc.running = true
  }
  ...
  function updateAdaptive(raw) {
    var parts = String(raw || "").trim().split("\t")
    adaptiveActive = parts[0] === "active"
    adaptiveCeiling = parts.length > 1 ? parts[1].trim() : ""
  ...
  function toggleAdaptive() {
    if (adaptiveToggleProc.running) return
    adaptiveToggleProc.command = ["sudo", "-n", "systemctl",
      root.adaptiveActive ? "disable" : "enable", "--now", "adaptive-charge.service"]
    adaptiveToggleProc.running = true
  }
        ...
        Column {
          ...
          Row {
            ...
            ToggleSwitch {
              ...
              foreground: root.bar.foreground
              anchors.verticalCenter: parent.verticalCenter
              onToggled: root.toggleAdaptive()

              PanelToolTip {
                visible: adaptiveSwitch.containsMouse
                text: root.adaptiveActive ? "Turn adaptive charging off" : "Turn adaptive charging on"
                fontFamily: root.bar.fontFamily
              }
            }
          }
